...
uses
  ColorMath, ColorHash, CoreMath, PointList, MatrixMath, MatrixOps;
    ...
    for Y:=0 to H do
      ...
      begin
        ...
        begin
          ...
          LAB.L := L1; LAB.A := a1; LAB.B := b1;
          LABDict.Add(ImgArr[Y][X], LAB);
        end;
        Result[y][x] := Sqr(LAB.L-L0) + Sqr(LAB.A-A0) + Sqr(LAB.B-B0);
      end;
    if (CCMode = CC_Euclid) then
      Result := fSqrt(Result)
    else if (CCMode = CC_EuclidNormed) then
  ...
  begin
    for Y:=0 to H do
      ...
      begin
        ...
        Result[y][x] := Abs(LAB.L-L0) + Abs(LAB.A-A0) + Abs(LAB.B-B0);
      end;
    if (CCMode = CC_ChebNormed) then
  ...
  begin
    for Y:=0 to H do
      ...
      begin
        ...
        Result[y][x] := LCHDiff(L0,a0,b0, LAB.L,LAB.A,LAB.B);
      end;
