...
function MergeATPA(const ATPA: T2DPointArray): TPointArray;
var
  i, ii, Len, TmpLen, TPALen: integer;
begin;
  Len := High(ATPA);
  if Len < 0 then Exit;
  TPALen := 0;
  ...
  begin
    ...
    if TmpLen < 0 then
      Continue;
    TmpLen := TmpLen + TPALen + 1;
    Setlength(Result, TmpLen+1);
    for ii := TPALen to TmpLen do
      Result[II] := ATPA[I][II - TPALen];
    TPALen := TmpLen;
  end;
  SetLength(Result,TPALen);
end;
...
{*
 ...
 //Future: Should look in to rotating the TPA first, then filling all small holes. Might be better.
*}
function RotateTPAEx(const TPA: TPointArray; const Center:TPoint; Radians: Extended): TPointArray; //StdCall;
var
   I, L,cx,cy,h: Integer;
