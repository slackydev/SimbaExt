...
begin
  ...
  for Y:=B.y1 to B.y2 do
    for X:=B.x1 to B.x2 do
      case Comparator of
        __LT__: if Mat[y][x] < Value then  Result[IncA(i)] := Point(x,y);
        __GT__: if Mat[y][x] > Value then  Result[IncA(i)] := Point(x,y);
        __EQ__: if Mat[y][x] = Value then  Result[IncA(i)] := Point(x,y);
        __LE__: if Mat[y][x] <= Value then Result[IncA(i)] := Point(x,y);
        __GE__: if Mat[y][x] >= Value then Result[IncA(i)] := Point(x,y);
        __NE__: if Mat[y][x] <> Value then Result[IncA(i)] := Point(x,y);
      end;
  SetLength(Result, i);
end;
